...
def show_condensator():
    ...
    st.info("**Ескерту:** Графиктерде көріп тұрғаныңыздай, өткізгіштің сыйымдылығы заряд пен кернеуге тәуелді емес. Ал жазық конденсатордың сыйымдылығы оның геометриялық өлшемдеріне (S, d) тікелей тәуелді.")