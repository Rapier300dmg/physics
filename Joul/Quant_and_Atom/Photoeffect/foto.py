...
def show_photoeffect():
    st.title("💡 Фотоэффект құбылысы")
    st.markdown("""
    ...
    *(мұнда $h\\nu$ - фотон энергиясы, $A_{шығу}$ - шығу жұмысы, $E_{к,max}$ - электронның макс. кинетикалық энергиясы)*
    ...
    # Түскен фотондар
    photon_color = f"rgb({min(255, 800-wavelength_nm)}, 0, {min(255, wavelength_nm-200)})"
    ...
    # Ұшып шыққан электрондар
    if photoeffect_happens:
        st.success(f"✅ Фотоэффект жүріп жатыр! E_ph ({e_photon_ev:.2f} эВ) > A_out ({a_out_ev:.2f} эВ)")
        ...
        st.error(f"❌ Фотоэффект жоқ. E_ph ({e_photon_ev:.2f} эВ) < A_out ({a_out_ev:.2f} эВ)")
    ...
    freq_range = np.linspace(3e14, 15e14, 100) 
    ...
    with g2:
        st.write("**Фототок пен Уақыт (t)**")
        fig_current_int = go.Figure()
        val = intensity if photoeffect_happens else 0
        fig_current_int.add_trace(go.Scatter(x=[0, 1, 2, 3, 4, 5], y=[0, val, val, val, val, val], 
                                           name="Фототок", line=dict(color='LimeGreen', width=3)))
        fig_current_int.update_layout(xaxis_title="Уақыт", yaxis_title="Фототок", yaxis=dict(range=[0, 25]), height=350)
        st.plotly_chart(fig_current_int, use_container_width=True)

    m1, m2 = st.columns(2)
    m1.metric("Фотон энергиясы (E_ph):", f"{e_photon_ev:.2f} эВ")
    m2.metric("Электрон энергиясы (E_k,max):", f"{max(0, e_kin_ev):.2f} эВ")